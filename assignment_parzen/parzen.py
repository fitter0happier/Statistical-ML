...
def parzen(x, x_trn, h):
    ...
    n = x_trn.shape[0]
    K_n = np.vectorize(lambda x : np.sum(norm.pdf(x - x_trn, loc=0, scale=h)) / n)
    p = K_n(x)
...
def compute_Lh(itrn, itst, x, h):
    ...
    for itst_i, itrn_i in zip(itst, itrn):
        Lh += np.sum(np.log(parzen(x[itst_i], x[itrn_i], h)))
...
def classify_bayes_parzen(x_test, xA, xC, pA, pC, h_bestA, h_bestC):
    ...
    labels = np.where(p_xA * pA >= p_xC * pC, 0, 1)
    return labels
